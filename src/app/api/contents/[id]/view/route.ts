...
export async function POST(
  ...
  try {
    ...
    console.log(`조회수 요청: ContentID=${contentId}, IP=${clientIp}, SessionID=${sessionId.substring(0, 8)}...`)
    ...
    console.log(`콘텐츠 확인: ID=${content.id}, 제목=${content.title}, 조회수=${content.view_count}`)

    // 세션 기반 중복 방지 체크
    if (!canCountView(sessionId, contentId)) {
      console.log(`조회수 중복 방지: ContentID=${contentId}, SessionID=${sessionId.substring(0, 8)}...`)
      return NextResponse.json({
        success: true,
    ...
    console.log(`업데이트 시도: ContentID=${contentId}, 현재 조회수=${content.view_count}, 새로운 조회수=${content.view_count + 1}`)
    ...
    console.log(`업데이트 시도 결과: error=${!!updateError}`)
    ...
    try {
      ...
      if (!refetchError && updatedContent) {
        ...
        console.log(`재조회 성공: 실제 view_count=${finalViewCount}, 기대값=${content.view_count + 1}`)
      } else {
        console.log('재조회 실패, 기대값 사용:', finalViewCount)
      }
    } catch (error) {
      console.log('재조회 오류, 기대값 사용:', finalViewCount, error)
    }
    
    // 조회 기록
    recordView(sessionId, contentId)
    
    console.log(`조회수 증가 완료: ContentID=${contentId}, 새로운 조회수=${finalViewCount}`)
